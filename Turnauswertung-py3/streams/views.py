...
def detail(request, id, slug):
    stream = (
        Stream.objects.prefetch_related("discipline_set")
    ...
    teams_disciplines_rank_dict = stream.get_teams_disciplines_rank_dict(
    ...
    context = {
        ...
        "team_performances_completed": completed_performances(
            teams_disciplines_result_dict
        ),
    }
...
def new(request):
    if request.method == "GET":
        ...
        stream = _build_stream_from_post(request, post_dict=request.POST)
        return redirect(stream.get_absolute_url())

    return HttpResponseNotAllowed(["GET", "POST"])


def _build_stream_from_post(request, stream=None, post_dict={}, method="create"):
    disciplines = Discipline.objects.all()

    # check if selected disciplines exist
    try:
        selected_disciplines = [
            disciplines.get(id=discipline_id)
    ...
    if method == "create":
        for position, discipline in enumerate(selected_disciplines):
            ...
            stream_discipline_join = StreamDisciplineJoin(
                stream=stream, discipline=discipline, position=position
            )
            stream_discipline_join.save()
    elif method == "update":
        stream_discipline_old_positions = stream.streamdisciplinejoin_set.all()
        for position, discipline in enumerate(selected_disciplines):
            stream_discipline_join = stream_discipline_old_positions.get(
