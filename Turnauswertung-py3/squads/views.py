...
def detail(request, id, slug):
    ...
    athletes = (
        squad.athlete_set.order_by("squad_position")
        .select_related("club")
    ...
    context = {
        ...
        "performances_completed": completed_performances(
...
def assign_athletes(request, id, slug):
    ...
    if request.method == "GET":
        ...
        non_squad_athletes = (
            Athlete.objects.filter(squad=None)
            .select_related("club")
            .select_related("stream")
            .select_related("team__stream")
            .select_related("squad")
        ...
        old_squad_athletes = squad.athlete_set.all().select_related("squad")
        ...
        new_squad_athletes = [
            ...
            for athlete_id in request.POST["chosen_list_order"].split()
        ]
        ...
        return redirect(reverse("squads.detail", kwargs={"id": id, "slug": slug}))
...
def enter_performances(request, id, slug):
    ...
    stream_discipline_tabindex_dict = {
        stream.id: {
            ...
            for discipline_index, discipline in enumerate(
                stream.get_ordered_disciplines()
            )
        }
    ...
    athletes_discipline_results = (
        squad.athlete_set.all()
    ...
    for result in athletes_discipline_results:
        athletes_disciplines_result_dict[result["id"]][
            result["performance__discipline_id"]
...
def handle_entered_performances(request, id, slug):
        ...
        if "-" in key and value:
            ...
            try:
                performance = performances.get(
                    athlete_id=athlete_id, discipline_id=discipline_id
                )
    ...
    return redirect(
        reverse("squads.detail", kwargs={"id": id, "slug": slug}) + "#performances"
    )
...
def create_judge_pdf(request):
    ...
    for squad in squads:
        ...
        for athlete in squad_athletes_dict[squad.id]:
            if previous_athlete and athlete.stream != previous_athlete.stream:
                squad_difficulty_indices[squad.id][
                    previous_athlete.id
                ] = athletes_in_stream
                athletes_in_stream = 0
    ...
    file_name = "{}_{}_{}.pdf".format(
        gettext_lazy("Squads"), gettext_lazy("Judge"), gettext_lazy("Lists")
    )
