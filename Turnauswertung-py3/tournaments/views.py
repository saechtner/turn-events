...
def create_certificates_pdf(request):
    ...
    file_name = "filename={}.pdf".format(gettext_lazy("Certificates"))
...
def create_solo_certificate_data_txt(request, id, slug):
    ...
    athletes = [
        athlete
        for athlete in sum(data.get("stream_athletes_dict").values(), [])
    ...
    athlete_rank_dict = {
        athlete.id: data.get("athlete_disciplines_rank_dict")
        .get(athlete.id)
        .get("total")
        for athlete in athletes
    }
...
def create_team_certificate_data_txt(request, id, slug):
    ...
    teams = [
        team
        for team in sum(data.get("stream_teams_dict").values(), [])
...
def create_evaluation_pdf(request, id, slug):
    ...
    team_format_dict = {
        team.id: -(len(team_athletes_dict[team.id]) + 1) for team in teams
    }

    club_stream_athlete_number_dict = {
        club.id: {
            stream.id: len(
                set(stream_athletes_dict.get(stream.id, []))
                & set(club.athlete_set.all())
            )
    ...
    # make pythonic
    for club in clubs:
        club_stream_athlete_number_dict[club.id]["total"] = sum(
            club_stream_athlete_number_dict.get(club.id, {}).values()
        )

    clubs = [
        club
        for club in clubs
        if club_stream_athlete_number_dict[club.id].get("total", 0) > 0
    ]
    ...
    context["total_athletes"] = sum(
        [len(athlete_list) for athlete_list in stream_athletes_dict.values()]
    )
