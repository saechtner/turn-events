...
class Tournament(models.Model):
    ...
    def get_solo_data_url(self):
        return reverse('tournaments.create_solo_data_txt', kwargs={ 'id': self.id, 'slug': self.slug })

    def get_team_data_url(self):
        return reverse('tournaments.create_team_data_txt', kwargs={ 'id': self.id, 'slug': self.slug })
    ...
    def get_evaluation_data(self):
        ...
        for stream in streams:
            ...
            stream_athletes_dict[stream.id] = [athlete for athlete in stream_athletes_dict[stream.id] if stream_athletes_disciplines_result_dict.get(athlete.id).get('total', 0.0) > 0.0]
            ...
            stream_athletes_dict[stream.id] = sorted(stream_athletes_dict.get(stream.id), key=lambda athlete: stream_athletes_disciplines_rank_dict.get(athlete.id).get('total'))
            stream_teams_dict[stream.id] = sorted(stream_teams_dict.get(stream.id), key=lambda team: stream_teams_disciplines_rank_dict.get(team.id).get('total'))
